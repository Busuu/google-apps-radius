...
module.exports.authenticate = authenticate =
    function(username, password, domain, options, callback) {
  if (!callback) {
    callback = options;
    options = {};
  }
...
module.exports.createServer = function (options) {
  ...
  server.on('message', function (msg, rinfo) {
    ...
    if (username.indexOf('@') !== -1) {
      // username contains domain part, strip it off and put it in
      // options.domain
      var uname_parts = username.split('@');
      username = uname_parts[0];
      options.domain = uname_parts[1];
    }
